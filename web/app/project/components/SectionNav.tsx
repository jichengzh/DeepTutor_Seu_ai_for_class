        ...
        {sections.map((sec) => {
          ...
          return (
            ...
            >
              ...
              {sec.hasRevision && (
                <span title={t("Modified")}><CheckCircle className="w-3.5 h-3.5 text-green-500 shrink-0" /></span>
              )}
              {sec.hasChat && !sec.hasRevision && (
                <span title={t("Has dialogue")}><MessageSquare className="w-3 h-3 text-amber-400 shrink-0" /></span>
              )}
            </button>
          );
        })}
