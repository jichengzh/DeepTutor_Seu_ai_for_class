    ...
    if content_list_file.exists():
        try:
            ...
            if target:
                ...
                # Update page range
                page_start = min(item.get("page_idx", page_start) for item in body_items if item.get("page_idx"))
                page_end = max(item.get("page_idx", page_end) for item in body_items if item.get("page_idx")) + 1
        except (json.JSONDecodeError, IOError):
